...
const AddClientTestimonialForm: React.FC<AddClientTestimonialFormProps> = ({ onClose }) => {
  ...
  const updateTestimonialMutation = useUpdateClientTestimonial();
  ...
  const uploadImage = async (file: File): Promise<string> => {
    ...
    const filePath = `client-testimonials/${fileName}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.code.trim()) {
      toast.error('Please provide a valid code');
      return;
    }
    ...
    try {
      ...
      toast.error('Failed to submit testimonial. Please check your code and try again.');
    } finally {
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-4">
              ...
              <Button
                type="submit"
                disabled={isUploading || updateTestimonialMutation.isPending}
                className="flex-1"
              >
                {isUploading || updateTestimonialMutation.isPending ? 'Submitting...' : 'Submit'}
              </Button>
            </div>
          </form>
        </div>
