...
const EditCertificateForm: React.FC<EditCertificateFormProps> = ({ isOpen, onClose, certificate }) => {
  ...
  // Populate form with certificate data when certificate changes
  useEffect(() => {
    if (certificate) {
      setFormData({
        ...
        title_color: (certificate.title_color as 'light' | 'dark') || 'light'
      });
      setImagePreview(certificate.certificate_image_card || null);
    }
  }, [certificate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (image) {
        ...
        updateData.certificate_image_card = imageUrl;
      }

      await updateCertificate(certificate.id, updateData);
